...
def render_detailpage(maori, id):
    if request.method == "POST":
        ...
        query3 = "Update Dictionary SET Maori=?, English=?, Category=?, Definition=?, Level=?, Time=? WHERE id=?"
        ...
        if Category not in Category_list:
            re_Category = True
        print(Maori, English, Category, Definition, Level, Time, id)
        if re_Category is True:
            return redirect("/edit?error=Category+is+invalid")
        else:
            cur3.execute(query3, (Maori, English, Category, Definition, Level, Time, id))
    ...
    query = "SELECT Maori, English, Category, Definition, Level, Image, Time FROM Dictionary where Maori=?"
    ...
    query2 = "SELECT Category, id FROM Category"
...
def render_edit_page():
    if request.method == "POST":
        ...
        query = "INSERT INTO Dictionary(Maori, English, Category, Definition, Level, Image, Time) VALUES(?,?,?,?,?,Null,?)"

        cur = con.cursor()

        Category_list = ["Actions", "Animals", "Clothing", "Culture and Religion", "Descriptive", "Emotions", "Food",
                         "Math and Number", "Outdoors", "People", "Places", "Plants", "School", "Sport", "Technology",
                         "Time", "Others"]
        re_Category = False
        if Category not in Category_list:
            re_Category = True
        ...
        if re_Category is True:
            ...
            cur.execute(query, (Maori, English, Category, Definition, Level, Time))
